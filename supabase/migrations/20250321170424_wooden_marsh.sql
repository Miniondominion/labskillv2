...
CREATE POLICY "skill_logs_insert_policy"
  ...
  TO public
  WITH CHECK (
    -- For self-logs: user must be the student
    (evaluated_student_id IS NULL AND auth.uid() = student_id) OR
    -- For peer evaluations: user must be the evaluator and a student must be specified
